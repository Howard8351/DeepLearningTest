...
class Deep_Q_Learning:
    def __init__(self, num_states, num_actions, num_hiddens, gamma,
        ...
        #target_model只被動接收權重
        #訓練模型
        self.train_model = creat_model(num_states, num_actions, num_hiddens)
        #目標模型
        self.target_model = creat_model(num_states, num_actions, num_hiddens)
        #設定最佳化方式
    ...
    def save_model(self):
        self.train_model.save('E:/python program/增強式學習結果/Model/DQL_Model',
                              include_optimizer = False)
            
def creat_model(n_states, n_actions, num_hidden):
    #建立一個模型架構能根據給予的資料(環境狀態)輸出最合適的動作
    #以這個例子動作只有物件向左或向右移動兩種
    #Deep Q Learning 是在做 Main model Q Value 對 Next Q Value 的近似化
    #所以輸出還是以線性輸出為主
    if type(num_hidden) == int:
        input_layer = Input(shape = (n_states,))
        layer = layers.Dense(num_hidden)(input_layer)
        layer = layers.PReLU()(layer)
        output_layer = layers.Dense(n_actions)(layer)   
    else:
        input_layer = Input(shape = (n_states,))
        layer = layers.Dense(num_hidden[0])(input_layer)
        layer = layers.PReLU()(layer)
        #layer = layers.Dense(num_hidden[0], activation='tanh', kernel_initializer='RandomNormal')(input_layer)
        for i in range(1, len(num_hidden)):
            layer = layers.Dense(num_hidden[i])(layer)
            layer = layers.PReLU()(layer)
            #layer = layers.Dense(num_hidden[i], activation='tanh', kernel_initializer='RandomNormal')(layer)
        output_layer = layers.Dense(n_actions)(layer)

    return Model(input_layer, output_layer)

def training_loop(epochs, num_states, num_actions, hidden, gamma, random_action_rate, reduce_rate, min_rate,
                 target_replace_count, target_replace_threshold, increase_rate, max_replace_threshold, memory_capacity,
                min_memory, batch_size, learning_rate):
    DQL_model = Deep_Q_Learning(num_states, num_actions, hidden, gamma,
                                memory_capacity, min_memory, batch_size, learning_rate)
    ...
    while DQL_model.get_memory_size() < (min_memory - 1):
        #取得模型選擇的動作
        action = DQL_model.get_action(state, 1.1)
    ...
    for epoch in range(epochs):
        ...
        if random_action_rate > min_rate:
            random_action_rate *= reduce_rate
        else:
            random_action_rate = min_rate
...
if __name__ == '__main__':
    ...
    gamma = 0.9
    #target network 更新間隔
    target_replace_count = 20
    target_replace_threshold = 50
    increase_rate = 0.5
    max_replace_threshold = 100
    ...
    step_list, hidden, learning_rate = training_loop(epochs, num_states, num_actions, hidden, gamma, random_action_rate,
